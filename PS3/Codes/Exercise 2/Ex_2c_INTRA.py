# -*- coding: utf-8 -*-
"""
Created on Tue Oct 27 21:21:35 2020
...
while progress > conv: #until value function sufficiently changes
    ...
    for j in gs:
        for i in gs:
            a = reward[i][j]+beta*valueold[i]
            if i>0 and a<m[i-1][j]:
                m[i][j]=m[i-1][j]
            else:
                m[i][j]=a
...
elapsed = time.time() - t
print("Ex2c runtime:", elapsed)
print("Ex2c no. iterations:", it)


plt.plot(ks,value)
plt.xlabel('Capital')
plt.ylabel('Value')
plt.title('Value function plot 2c_INTRA')
plt.savefig('Value function plot 2c_INTRA.png')
plt.show()

plt.plot(ks,l)
plt.xlabel('Capital')
plt.ylabel('Labour')
plt.title('Labour policy function plot 2c_INTRA')
plt.savefig('Labour policy function plot 2c_INTRA.png')
plt.show()

plt.plot(ks,cons)
plt.xlabel('Capital')
plt.ylabel('Consumption')
plt.title('Consumption policy function plot 2c_INTRA')
plt.savefig('Consumption policy function plot 2c_INTRA.png')
plt.show()

plt.plot(ks,kp)
plt.xlabel('Capital')
plt.ylabel('Capital policy function')
plt.title('Capital policy function plot 2c_INTRA')
plt.savefig('Capital policy function plot 2c_INTRA.png')
plt.show()
