...
while progress > conv: #until value function sufficiently changes
    ...
    for j in gs:
        for i in gs:
            m[i][j] = reward[i][j]+beta*valueold[i]
...
elapsed = time.time() - t
print("Ex2a_LOOP runtime:", elapsed)
print("Ex2a_LOOP no. iterations:", it)

plt.plot(ks,value)
plt.xlabel('Capital')
plt.ylabel('Value function')
plt.title('Value function plot 2a_LOOP')
plt.savefig('Value function plot 2a_LOOP.png')
plt.show()    

plt.plot(ks,l)
plt.xlabel('Capital')
plt.ylabel('Labour')
plt.title('Labour policy function plot 2a_LOOP')
plt.savefig('Labour policy function plot 2a_LOOP.png')
plt.show()

plt.plot(ks,cons)
plt.xlabel('Capital')
plt.ylabel('Consumption')
plt.title('Consumption policy function plot 2a_LOOP')
plt.savefig('Consumption policy function plot 2a_LOOP.png')
plt.show()

plt.plot(ks,kp)
plt.xlabel('Capital')
plt.ylabel('Capital policy function')
plt.title('Capital policy function plot 2a_LOOP')
plt.savefig('Capital policy function plot 2a_LOOP.png')
plt.show()
